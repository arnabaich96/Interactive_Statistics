...
### What percentage of people drink more than 13 liters of coffee per month?
...
ggplot(data.frame(x, y), aes(x, y)) +
  geom_line(color = "darkblue",linewitdth=1) +
...
From the standard normal table, the probability of a z-score greater than 1 is 0.0668. Therefore, approximately **6.68%** of people drink more than 13 liters of coffee per month.

---

###  What percentage of people drink less than 6.5 liters of coffee per month?
...
**Solution**: The z-score for 6.5 liters is:

$$ Z = \frac{6.5 - 10}{2} = -1.75 $$

From the standard normal table, the probability of a z-score less than -1 is 0.04. Therefore, approximately **4%** of people drink less than 6.5 liters of coffee per month.
...
**Solution**: The area between \( Z = -0.5 \) and \( Z = 1 \) is approximately **53.2%** .
...
## Detecting Normality: Q-Q Plot

A **Q-Q (Quantile-Quantile) Plot** is a graphical tool used to compare the quantiles of a sample dataset to the quantiles of a theoretical distribution, such as the normal distribution.

### How to Make a Q-Q Plot?

To construct a Q-Q plot, follow these steps:

1. **Sort the Data**: Arrange the data points in increasing order.
2. **Calculate the Theoretical Quantiles**: For a normal Q-Q plot, calculate the theoretical quantiles from the standard normal distribution.
3. **Plot**: Plot the sorted data points against the theoretical quantiles. If the data is normally distributed, the points should fall on or near a straight line.

### Mathematical Details:

Given a dataset with \( n \) values \( X_1, X_2, ..., X_n \), the steps are as follows:

1. Order the data such that \( X_{(1)} \leq X_{(2)} \leq ... \leq X_{(n)} \).
2. The \( i \)-th theoretical quantile for a standard normal distribution is computed as:

\[
Q_i = z_{ \left(1 - \frac{i - 0.5}{n} 
\right)}
\]

where \( z_{\alpha} \) is a point on the standard normal distribution corresponding to have the probability \( \alpha \)    over it and \( i = 1, 2, ..., n \).

3. Plot the ordered values \( X_{(i)} \) against the theoretical quantiles \( Q_i \).

### How to Interpret a Q-Q Plot?

- **Points lie on a straight line**: The data follows a normal distribution.
- **Points curve away from the line**: This suggests non-normality. The nature of the deviation (e.g., upward or downward curves) indicates whether the distribution is skewed or has heavy/light tails.

### Example: Q-Q Plot with 10 Randomly Generated Points

Let’s generate 10 random points from a normal distribution and construct a Q-Q plot. We will also calculate and display the theoretical quantiles and observed values in a table.
...
# Compute theoretical quantiles for standard normal distribution
n <- length(sorted_data)
theoretical_quantiles <- qnorm((1:n - 0.5) / n)

# Create a data frame to display in the HTML table
data_frame <- t(data.frame(
  "Ordered Data" = sorted_data,
...
data_frame %>%
  ...
  column_spec(1, bold = T, color = "white", background = "#7C0A02") %>%
  column_spec(2:11, background = "lightblue")

```

Now, let’s generate the Q-Q plot for these 10 points.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Q-Q plot for the 10 random points
ggplot(data.frame(x = theoretical_quantiles, y = sorted_data), aes(x = x, y = y)) +
  geom_point(color = "blue", size = 3) +
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed",size = 1) +
  labs(title = "Q-Q Plot for 10 Random Points", x = "Theoretical Quantiles", y = "Ordered Data") +
  theme_minimal()
```

### Different Q-Q plots:

Let’s explore how Q-Q plots look for different distributions:

1. **Positive Skew (Right-Skewed)**: Data with a long tail to the right.
2. **Negative Skew (Left-Skewed)**: Data with a long tail to the left.
3. **Symmetric**: Symmetrical data, possibly following a normal distribution.
4. **Uniform Distribution**: Data uniformly distributed between two points.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Generate right-skewed data (chi-squared distribution)
right_skewed_data <- rchisq(1000, df = 3)

# Q-Q plot for right-skewed data 
qqrs<- ggplot(data.frame(x = right_skewed_data), aes(sample = x)) +
  stat_qq(color = "red") +
  stat_qq_line(color = "blue",size =1) +
  labs(title = "Positively Skewed Data") +
  theme_minimal()

# Generate left-skewed data (negatively skewed)
left_skewed_data <- -rchisq(1000, df = 3)

# Q-Q plot for left-skewed data
qqls <- ggplot(data.frame(x = left_skewed_data), aes(sample = x)) +
  stat_qq(color = "green") +
  stat_qq_line(color = "blue",size =1) +
  labs(title = "Negatively Skewed Data") +
  theme_minimal()

# Generate symmetric data (t-distribution with df = 10)
symmetric_data <- rt(1000, df = 10)

# Q-Q plot for symmetric data
qqsym <- ggplot(data.frame(x = symmetric_data), aes(sample = x)) +
  stat_qq(color = "purple") +
  stat_qq_line(color = "blue",size =1) +
  labs(title = " Symmetric Data") +
  theme_minimal()

# Generate uniformly distributed data
uniform_data <- runif(1000)

# Q-Q plot for uniformly distributed data
qquni <- ggplot(data.frame(x = uniform_data), aes(sample = x)) +
  stat_qq(color = "orange") +
  stat_qq_line(color = "blue",size =1) +
  labs(title = "Uniformly Distributed Data") +
  theme_minimal()


# plot the Q-Q plots side by side
(qqrs + qqls)/(qqsym + qquni)
...
- **Symmetricity**:
  The normal distribution is symmetric around the mean, with 50% of the data falling below the mean and 50% above the mean. So for a std normal random variable Z
  $$z_{\alpha} = - z_{1-\alpha} \text{   and   } P(Z > x) = P(Z < -x).$$
 This property can be verified for Normal distribution with mean $\mu$ and variance $\sigma^2$ and we can have the following.
  $$P(X > \mu + x) = P(X < \mu - x)$$

- **QQ Plot**:
  A Q-Q plot is a graphical tool used to compare the quantiles of a sample dataset to the quantiles of a theoretical distribution to check for normality.
...
### Question 11:
**How can you use a Q-Q plot to detect skewness in data?**

**Solution**:
Skewness can be detected in a Q-Q plot by observing how the points deviate from the straight line:
- **Positive skewness (right-skewed):** The points will curve upward, indicating a long tail to the right.
- **Negative skewness (left-skewed):** The points will curve downward, indicating a long tail to the left.

---

### Question 12:
**Why might a Q-Q plot for a small sample size show more deviation from the straight line?**

**Solution**:
With smaller sample sizes, random variation can cause more deviation from the expected theoretical distribution in a Q-Q plot. This does not necessarily mean that the data does not follow the distribution; rather, it reflects the increased variability and noise present in smaller datasets. As the sample size increases, the points in the Q-Q plot will tend to align more closely with the straight line if the data follows the theoretical distribution.

---

### Question 13:
**Can we detect outliers from a Normal Q-Q plot?**
**Solution**:
Yes, outliers can be detected from a Normal Q-Q plot. Outliers are data points that deviate significantly from the expected pattern of the Q-Q plot. They may appear as points that are far away from the straight line, indicating that these values do not follow the normal distribution pattern of the data.
